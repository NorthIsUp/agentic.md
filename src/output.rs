...
pub struct OutputResult {
    ...
    /// Subset of `stale`: generated files whose on-disk content has merge conflict markers.
...
pub fn check(files: &[GeneratedFile], conflict_regen: bool) -> OutputResult {
    ...
    for file in files {
        ...
        if has_generated_by_marker(&file.path) {
            if conflict_regen && existing.contains(CONFLICT_MARKER) {
                result.conflicted.push(file.path.clone());
            }
            result.stale.push(file.path.clone());
        } else {
...
pub fn fix(files: &[GeneratedFile], overwrite: bool, conflict_regen: bool) -> OutputResult {
    let mut result = OutputResult::default();
    ...
    for file in files {
        ...
        if let Some(ref existing) = existing_content {
            // Skip files we don't own, unless --overwrite
            if !overwrite && !has_generated_by_marker_in(existing) {
        ...
        match std::fs::write(&file.path, &file.content) {
            Ok(()) => {
                if conflict_regen
                    && existing_content
                        .as_deref()
                        .is_some_and(|s| s.contains(CONFLICT_MARKER))
                {
                    result.conflicted.push(file.path.clone());
                }
...
fn has_generated_by_marker(path: &Path) -> bool {
    match std::fs::read_to_string(path) {
        Ok(content) => has_generated_by_marker_in(&content),
        Err(_) => false,
    }
}

fn has_generated_by_marker_in(content: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn check_detects_conflict_markers() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join(".cursor/rules/test.mdc");
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        ...
        // With conflict_regen=false, no conflicted entries
        let result2 = check(&files, false);
        assert!(result2.conflicted.is_empty());
        assert_eq!(result2.stale.len(), 1);
    }
    ...
    fn fix_overwrites_unowned_when_forced() {
        ...
        let result = fix(&files, true, true);
        assert_eq!(result.written.len(), 1);
        assert!(result.skipped.is_empty());
    ...
    #[test]
    fn fix_resolves_conflict_markers() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join(".cursor/rules/conflict.mdc");
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        let conflicted = concat!(
            "---\ngenerated-by: agentic-sync\n---\n",
            "<<<<<<< HEAD\nContent A\n=======\nContent B\n>>>>>>> branch\n"
    ...
    #[test]
    fn fix_skips_conflict_when_no_conflict_regen() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join(".cursor/rules/conflict.mdc");
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        let conflicted = concat!(
        ...
        let result = fix(&files, false, false);
        assert_eq!(result.skipped.len(), 1);
        assert!(result.written.is_empty());
        // Conflict preserved
